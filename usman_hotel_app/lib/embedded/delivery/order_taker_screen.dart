...
class _OrderTakerScreenState extends State<OrderTakerScreen> {
  ...
  /// Saves a delivery order locally when the server is unreachable and returns
  /// a display-ready order map so the UI/print flow keeps working.
  ...
  Future<void> markPaid(Map order) async {
    final gid = sOf(order['mergeGroupId']);
    if (gid.isNotEmpty) {
      final members = orders
          .whereType<Map>()
          .map((e) => Map<String, dynamic>.from(e))
          .where((o) => sOf(o['mergeGroupId']) == gid)
          .toList();
      if (members.length >= 2) {
        await _bulkMarkPaidList(members);
        return;
      }
    }
  ...
  Future<void> markReserved(Map order) async {
    await _putOrder(order, {
      'reserved': true,
  ...
  Future<void> unmarkReserved(Map order) async {
    await _putOrder(order, {
      'reserved': false,
      'reservedAt': '',
    });
  ...
  Future<void> _bulkDelete() async => _bulkDeleteList(_selectedOrdersList);

  Future<void> _bulkDeleteList(List<Map<String, dynamic>> list) async {
  ...
  Future<void> _bulkRider() async => _bulkRiderList(_selectedOrdersList);

  Future<void> _bulkRiderList(List<Map<String, dynamic>> list) async {
    if (list.isEmpty) return;
  ...
  Future<void> _bulkMarkPaid() async => _bulkMarkPaidList(_selectedOrdersList);

  Future<void> _bulkMarkPaidList(List<Map<String, dynamic>> list) async {
    if (list.isEmpty) return;
    if (_busy) return;
    setState(() => _busy = true);
    final ids = list.map((o) => sOf(o['id'])).where((x) => x.isNotEmpty).toList();
  ...
  Future<void> _bulkMarkDue() async => _bulkMarkDueList(_selectedOrdersList);

  Future<void> _bulkMarkDueList(List<Map<String, dynamic>> list) async {
    if (list.isEmpty) return;
    if (_busy) return;
    setState(() => _busy = true);
    final ids = list.map((o) => sOf(o['id'])).where((x) => x.isNotEmpty).toList();
  ...
  Future<void> _bulkMarkReservedList(List<Map<String, dynamic>> list) async {
    ...
    try {
      ...
      toast(offline
          ? '${ids.length} order(s) marked reserved (offline - auto-sync \u{1F512})'
          : '${ids.length} order(s) marked reserved \u{1F512}');
      await _refreshOrdersOnly();
    } catch (e) {
      toast(e.toString(), seconds: 6);
    } finally {
  ...
  List<Map<String, dynamic>> get deliveryAssigned =>
      deliveryOrders.where((o) {
        final st = sOf(o['status']).toLowerCase();
        return !_isReserved(o) &&
            sOf(o['deliveryAgent']).trim().isNotEmpty &&
            !_isDeliveryPaid(o) &&
            !_isDeliveryDue(o) &&
            !_isDeliveredOrDone(o) &&
            st != 'cancelled';
  ...
  List<Map<String, dynamic>> get deliveryReserved => deliveryOrders
      .where((o) =>
          _isReserved(o) &&
          sOf(o['status']).toLowerCase() != 'cancelled')
      .toList();
  ...
  List<Map<String, dynamic>> get visibleOrders {
    ...
    if (ordersSubTab == 'reserved') return deliveryReserved;
  ...
  List<Map<String, dynamic>> get filteredVisibleOrders {
    ...
    return visibleOrders
        .where((o) => ordersSubTab == 'reserved' || _inOrdersDateRange(o))
        .where((o) {
  ...
  /// Manual "Refresh" — sirf orders reload hota hain (fast, 1 endpoint).
  Future<void> _manualRefreshOrders() async {
    if (refreshBusy || syncing) return;
    setState(() => refreshBusy = true);
    await _refreshOrdersOnly();
    if (mounted) {
      setState(() => refreshBusy = false);
      toast('Orders refreshed ✅', seconds: 2);
    }
  }

  /// Manual "Full App Refresh" — saara data (categories, products, orders,
  /// settings, riders, staff) dobara load hota hai, fast.
  Future<void> _manualFullRefresh() async {
    if (refreshBusy || syncing) return;
    setState(() => refreshBusy = true);
    toast('Loading latest app data...');
    await _loadData();
    if (mounted) {
      setState(() => refreshBusy = false);
      toast('App refreshed ✅', seconds: 2);
    }
  }
            ...
            if (syncing)
              const SizedBox(
                ...
                child: CircularProgressIndicator(
                    strokeWidth: 2, color: Colors.white, valueColor: AlwaysStoppedAnimation(Colors.white)),
              ),
          ],
        ),
      ),
    ...
    return _PressFx(
      ...
      builder: (_, pressed) => Container(
        ...
        decoration: BoxDecoration(
            color: pressed ? fg.withValues(alpha: 0.28) : bg,
  ...
  Widget _rsBtn(String icon, String label, VoidCallback onTap, bool busy) {
    return _PressFx(
      enabled: !busy,
      onTap: onTap,
      builder: (_, pressed) => Container(
        ...
        decoration: BoxDecoration(
          color: busy
              ...
              : (pressed ? const Color(0xFFDCFCE7) : const Color(0xFFF1F5F9)),
          borderRadius: BorderRadius.circular(10),
  ...
  Widget _ordersScreen() {
    ...
    final subTabs = [
      ...
      ['reserved', 'Reserved (${deliveryReserved.length})'],
    ];
    final list = filteredVisibleOrders;
    final grouped = _groupOrderItems(list);
    return Scaffold(
      backgroundColor: Colors.white,
      body: SafeArea(
        child: Column(children: [
          Container(
            ...
            child: Row(children: [
              _rsBtn('\u{1F5D1}\uFE0F', 'Refresh', _manualRefreshOrders,
                  refreshBusy || syncing),
              const SizedBox(width: 6),
              _rsBtn('\u{1F504}', 'Sync', () {
                if (!syncing) unawaited(_syncPendingOrders());
              ...
              const SizedBox(width: 6),
              _rsBtn('\u{1F680}', 'Full', _manualFullRefresh,
                  refreshBusy || syncing),
              const Spacer(),
              IconButton(
                onPressed: () => setState(() {
          ...
          Padding(
            ...
            child: TextField(
              ...
              decoration: InputDecoration(
                ...
                suffixIcon: ordersSearch.isEmpty
                    ? null
                    : IconButton(
          ...
          Expanded(
            child: grouped.isEmpty
                ...
                : ListView.separated(
                    ...
                    itemBuilder: (_, i) {
                      final it = grouped[i];
                      if (it is Map<String, dynamic> && it['groupId'] != null) {
                        return _mergedOrdersCard(
                          groupId: sOf(it['groupId']),
                          members: List<Map<String, dynamic>>.from(it['members'] as List),
                        );
                      }
                      return _orderCard(it as Map<String, dynamic>);
                    },
                  ),
          ),
        ]),
  ...
  List<dynamic> _groupOrderItems(List<Map<String, dynamic>> list) {
    ...
    for (final g in groups.keys) {
      groups[g]!.sort((a, b) {
        final ta = DateTime.tryParse(sOf(a['createdAt'])) ??
            DateTime.fromMillisecondsSinceEpoch(0);
        final tb = DateTime.tryParse(sOf(b['createdAt'])) ??
            DateTime.fromMillisecondsSinceEpoch(0);
    ...
    return [
      ...singles,
      ...groups.entries.map((e) => {'groupId': e.key, 'members': e.value}),
    ];
  }

  Widget _mergedOrdersCard({
    required String groupId,
    required List<Map<String, dynamic>> members,
  }) {
    final ids = members.map((o) => sOf(o['id'])).toSet();
    final allSel = ids.every((id) => _selectedOrderIds.contains(id));
    final someSel = ids.any((id) => _selectedOrderIds.contains(id));
    final total =
        members.fold<num>(0, (s, o) => s + numOf(o['total'] ?? o['amount']));
    final first = members.first;
    final addr = sOf(first['address']);
    final expanded = expandedOrderId == groupId;
    final riders = members
        .map((o) => sOf(o['deliveryAgent']))
    ...
    return Container(
      decoration: BoxDecoration(
        ...
        boxShadow: [
          BoxShadow(color: const Color(0xFF7C3AED).withValues(alpha: 0.18),
      ...
      child: Column(children: [
        ListTile(
          onTap: () {
            if (selectMode) {
              ...
              setState(() => expandedOrderId = expanded ? null : groupId);
            }
          },
          leading: selectMode
              ? Icon(
                  allSel
                      ? Icons.check_circle
                      : (someSel ? Icons.remove_circle_outline : Icons.radio_button_off),
                  color: (allSel || someSel)
                      ? const Color(0xFF7C3AED)
                      : const Color(0xFF94A3B8),
                  size: 26)
              : const Icon(Icons.call_merge, color: Color(0xFF7C3AED), size: 26),
          title: Text('Merged Order \u2022 ${members.length} orders',
              style: const TextStyle(
                  fontSize: 13, fontWeight: FontWeight.w900, color: Color(0xFF6D28D9))),
          subtitle: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            if (riders.isNotEmpty)
              Row(children: [
                const Icon(Icons.delivery_dining, size: 13, color: Color(0xFF1D4ED8)),
                const SizedBox(width: 4),
                Expanded(
                  child: _glowText(riders.join(', '),
                      color: const Color(0xFF1D4ED8), size: 13),
                ),
              ]),
            if (addr.isNotEmpty)
              Padding(
          ...
          trailing: Column(
            ...
            children: [
              ...
              const Text('MERGED',
                  style: TextStyle(
                      fontSize: 9, fontWeight: FontWeight.w900, color: Color(0xFF4C1D95))),
            ],
          ),
        ),
        if (!selectMode &&
            !_isCashier() &&
            members.any((o) => !_isDeliveryPaid(o)))
          Padding(
        ...
        if (expanded) ...[
          ...
          ...members.map(_mergedMemberRow).toList(),
          Padding(
            ...
            child: Wrap(spacing: 8, runSpacing: 8, children: [
              ...
              _actBtn('Assign Rider', const Color(0xFF2563EB),
                  () => _bulkRiderList(members)),
              _actBtn('Print All', const Color(0xFF0EA5E9),
                  () => _printGroup(members)),
              if (_isManager())
                _actBtn('Delete', const Color(0xFFB91C1C),
                    () => _bulkDeleteList(members)),
            ]),
          ),
        ],
      ]),
  ...
  Widget _mergedMemberRow(Map<String, dynamic> o) {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Expanded(
            child: Text(
                '${sOf(o['status'])} \u00B7 ${sOf(o['paymentStatus'])}',
                style: const TextStyle(
                    fontSize: 12, fontWeight: FontWeight.w800, color: Color(0xFF1E293B))),
          ),
          const SizedBox(width: 6),
          Text('${_fmtNum(total)} PKR',
              style: const TextStyle(
        ...
        if (rider.isNotEmpty)
          Padding(
            ...
            child: Row(children: [
              ...
              const SizedBox(width: 4),
              Expanded(child: _glowText(rider, color: const Color(0xFF1D4ED8), size: 13.5)),
            ]),
          ),
        ...
        if (loc.isNotEmpty)
          Padding(
            padding: const EdgeInsets.only(top: 3),
            child: Row(children: [
              const Icon(Icons.location_on, size: 13, color: Color(0xFF2563EB)),
              const SizedBox(width: 4),
              Expanded(child: Text(loc, style: const TextStyle(fontSize: 11, color: Color(0xFF64748B)))),
            ]),
          ),
        if (addr.isNotEmpty)
          Padding(
            ...
            child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const Icon(Icons.home_work_outlined, size: 13, color: Color(0xFF059669)),
              const SizedBox(width: 4),
              Expanded(child: Text(addr, style: const TextStyle(fontSize: 11, color: Color(0xFF64748B)))),
            ]),
          ),
  ...
  Future<void> _printGroup(List<Map<String, dynamic>> members) async {
    for (final o in members) {
      try {
        await printOrderBT(o);
      } catch (e) {
        toast('Print failed: $e', seconds: 6);
        return;
      }
    }
    toast('Printed ${members.length} orders');
  }
  ...
  Widget _orderCard(Map<String, dynamic> o) {
    ...
    final sel = selectMode && _selectedOrderIds.contains(id);
    final reservedTab = ordersSubTab == 'reserved';
    final glow = reservedTab ? const Color(0xFF991B1B) : _orderStatusColor(o);
      ...
      child: Container(
        ...
        child: Column(children: [
              ...
              child: Row(children: [
                ...
                Expanded(
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    ...
                    const SizedBox(height: 2),
                    Text(
                      '${type.isNotEmpty ? '$type \u00B7 ' : ''}${_isManager() && ot.isNotEmpty ? '$ot \u00B7 ' : ''}${time12(o['createdAt'], utcOffset: esc.timeZoneOffsetFor(btSettings['receiptTimeZone']?.toString()))}',
                      style: const TextStyle(fontSize: 11, color: Color(0xFF64748B)),
                    ),
                  ]),
                ),
                ...
                Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                  ...
                  if (sOf(o['reserved']) == 'true' || o['reserved'] == true)
                    Container(
                      margin: const EdgeInsets.only(top: 3),
                      padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2),
                      decoration: BoxDecoration(
                          color: const Color(0xFF991B1B), borderRadius: BorderRadius.circular(100)),
          ...
          Padding(
            ...
            child: Row(children: [
              Icon(Icons.receipt_long, size: 13, color: reservedTab ? const Color(0xFF991B1B) : const Color(0xFF64748B)),
              const SizedBox(width: 4),
              Text('Order #$num',
                  style: TextStyle(fontSize: 11, fontWeight: FontWeight.w800, color: reservedTab ? const Color(0xFF991B1B) : const Color(0xFF64748B))),
              const Spacer(),
              if (selectMode)
                Text('Tap to select',
                    style: TextStyle(fontSize: 10, color: reservedTab ? const Color(0xFF991B1B) : const Color(0xFF94A3B8))),
            ]),
          ),
        ]),
      ),
  ...
  Widget _deliveryPill() => Container(
        ...
        child: const Row(mainAxisSize: MainAxisSize.min, children: [
          Icon(Icons.delivery_dining, size: 12, color: Color(0xFF1D4ED8)),
          SizedBox(width: 3),
          Text('DELIVERY',
              style: TextStyle(
                  fontSize: 9,
  ...
  Widget _actBtn(String label, Color c, VoidCallback? onTap, {bool glow = false}) => _PressFx(
        enabled: onTap != null && !_busy,
        builder: (_, pressed) => ElevatedButton(
          onPressed: onTap,
          style: ElevatedButton.styleFrom(
              backgroundColor: pressed ? _shadeDown(c) : c,
              foregroundColor: Colors.white,
  ...
  Color _shadeDown(Color c) {
    final hsv = HSVColor.fromColor(c);
    return hsv.withValue(hsv.value * 0.82).toColor();
  }
