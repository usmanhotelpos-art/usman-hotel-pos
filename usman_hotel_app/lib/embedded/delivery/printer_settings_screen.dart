...
class _OrderTakerPrinterSettingsState extends State<OrderTakerPrinterSettings> {
  ...
  String _receiptTimeZone = 'Karachi (UTC+5)';
  ...
  Future<void> _loadSettings() async {
    ...
    _receiptTimeZone = prefs.getString('ot_timeZone') ?? 'Karachi (UTC+5)';
  ...
  Future<void> _saveSettings() async {
    ...
    try {
      ...
      await prefs.setString('ot_timeZone', _receiptTimeZone);
  ...
  Map<String, dynamic> _buildSettingsMap() {
    final m = <String, dynamic>{
      ...
      'receiptTimeZone': _receiptTimeZone,
  ...
  Widget _buildTimeRegionCard() {
    return _card(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _title('🕐', 'Time & Date Region'),
          _drop('Time Region', _receiptTimeZone, [
            'Karachi (UTC+5)',
            'Islamabad (UTC+5)',
            'Lahore (UTC+5)',
          ], (v) => setState(() => _receiptTimeZone = v)),
          const Padding(
            padding: EdgeInsets.only(top: 4),
            child: Text('Receipt aur order time Pakistan Standard Time (UTC+5) mein dikhai dega.', style: TextStyle(fontSize: 12, color: txtDim)),
          ),
        ],
      ),
    );
