...
class _RidersOrdersSheetState extends State<_RidersOrdersSheet> {
  ...
  double _totalOf(String id) {
    for (final o in widget.orders) {
      if (sOf(o['id']) == id) return numOf(o['total'] ?? o['amount']);
    }
    return 0;
  }

  Future<void> _bulkMark(String payment) async {
    final ids = _selected.toList();
    if (ids.isEmpty) return;
    final markPaid = payment == 'paid';
    setState(() => _busy = true);
    try {
      for (final id in ids) {
        await ApiClient.send(
        ...
        if (markPaid) {
          await ApiClient.send('POST', '/pos/payments', token: widget.token, body: {
            ...
            'paymentMethod': 'Cash',
            'status': 'Completed',
            'description': 'Due payment for order $id',
          }).catchError((_) {});
        }
      }
      if (!mounted) return;
      ...
      ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(markPaid ? 'Marked as paid' : 'Marked as due')));
    } catch (e) {
      if (mounted) {
        setState(() => _busy = false);
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Failed: $e')));
                    ...
                    children: [
                      Expanded(
                        child: FilledButton.icon(
                          onPressed: _busy ? null : () => _bulkMark('due'),
                          icon: _busy
                      ...
                      if (widget.isManager) ...[
                        ...
                        Expanded(
                          child: FilledButton.icon(
                            onPressed: _busy ? null : () => _bulkMark('paid'),
                            icon: _busy
                                ? const SizedBox(width: 14, height: 14, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white))
