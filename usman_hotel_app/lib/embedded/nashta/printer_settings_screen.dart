...
const List<(String, int)> _tzOptions = [
  ('Pakistan \u2014 Karachi (PKT +5)', 5),
  ('Pakistan \u2014 Lahore (PKT +5)', 5),
  ('Pakistan \u2014 Islamabad (PKT +5)', 5),
];
...
class _OrderTakerPrinterSettingsState extends State<OrderTakerPrinterSettings> {
  ...
  String _timezone = 'Pakistan \u2014 Lahore (PKT +5)';
  bool _saving = false;
  ...
  Future<void> _loadSettings() async {
    ...
    _timezone = prefs.getString('ot_timezone') ?? 'Pakistan \u2014 Lahore (PKT +5)';
  ...
  int get _tzOffset {
    for (final t in _tzOptions) {
      if (t.$1 == _timezone) return t.$2;
    }
    return 5;
  }
  ...
  Future<void> _saveSettings() async {
    ...
    try {
      ...
      await prefs.setString('ot_timezone', _timezone);
    ...
    return _card(
      child: Column(
        ...
        children: [
          ...
          _drop('Time Zone', _timezone, _tzOptions.map((t) => t.$1).toList(), (v) => setState(() => _timezone = v)),
