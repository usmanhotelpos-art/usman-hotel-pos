package com.usmanhotel.delivery_orders_app
...
class MainActivity : FlutterActivity() {
    ...
    private fun connectPermissionGranted(): Boolean {
        ...
        } else true // Legacy Androids only need the (manifest-level) BLUETOOTH permission.
    }
                ...
                when (call.method) {
                    ...
                    "connect" -> {
                        ...
                        Thread {
                            ...
                            if (err != null) {
                                android.util.Log.e("UsmanBT", "connect($mac): $err")
                            }
                        }.start()
                    }
                    ...
                    "writeBytes" -> {
                        ...
                        // Dart sends Uint8List which arrives as byte[];
                        // accept every possible shape defensively.
                        ...
                        Thread {
                            val ok = try {
                                ...
                                android.util.Log.e("UsmanBT", "write failed: ${e.message}")
                                closeSocketQuietly()
                                false
                            }
    ...
    /** Closes any half-dead socket first - otherwise new connects get refused. */
    ...
    /**
     * Opens an SPP connection to [mac] trying three transports in order:
     *  1. standard secure RFCOMM,
     *  2. insecure RFCOMM (some cheap printers reject secure),
     *  3. reflection channel-1 fallback (classic workaround for
     *     "Service discovery failed" on many devices).
    ...
    private fun connectSpp(mac: String): String? {
        ...
        try {
            ...
            // Discovery eats the socket handshake - always cancel it first.
            ...
            // 3) Reflection fallback (channel 1)
            try {
