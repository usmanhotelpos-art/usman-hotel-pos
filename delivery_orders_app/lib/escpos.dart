...
int timeZoneOffsetFor(String? region) {
  final r = sOf(region).trim().toLowerCase();
  if (r.isEmpty) return 5;
  if (r.contains('karachi') || r.contains('islamabad') || r.contains('lahore') || r.contains('pakistan')) return 5;
  return 0;
}

DateTime applyTimeZone(DateTime date, int offsetHours) {
  if (offsetHours == 0) return date;
  if (date.isUtc) return date.add(Duration(hours: offsetHours));
  return date.toUtc().add(Duration(hours: offsetHours));
}

String formatDate(String? dateString, String? format, {int utcOffset = 5}) {
  if (dateString == null || dateString.isEmpty) return '';
  if (format == null || format.isEmpty) return '';
  final parsed = DateTime.tryParse(dateString);
  if (parsed == null) return '';
  final date = applyTimeZone(parsed, utcOffset);
...
List<int> buildEscposReceipt(
  ...
  final dateText = formatDate(
    ...
    utcOffset: timeZoneOffsetFor(settings['receiptTimeZone']?.toString()),
  );
