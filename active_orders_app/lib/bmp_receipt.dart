...
class _Painter {
  ...
  _Painter(this.canvas, this.pxWidth, {this.defaultAlign = ui.TextAlign.left});
...
Future<Uint8List> buildBmpReceipt(
  ...
  final totalDueFontSize = nOf(settings['btTotalDueFontSize']) != 0
      ...
      : 22.0;
  final totalDueBold = settings['btTotalDueBold'] != false;
  ...
  final tokenLabelFontSize = nOf(settings['btTokenLabelFontSize']) != 0
      ...
      : 14.0;
  final alignRaw = sOf(settings['btTextAlign'], def: 'center');
  ...
  final subtotal = subtotalOf(order);
  ...
  P.printLine('Total Due: ${fnum(totalAmount)} Rs',
      bold: totalDueBold,
      fontSize: totalDueFontSize,
      baseFontSize: baseFontSize);
  if (subtotal > 0) {
    P.printLine('Subtotal: ${fnum(subtotal)} Rs',
        fontSize: infoSz, baseFontSize: baseFontSize);
  }
  if (deliveryCharge > 0) {
