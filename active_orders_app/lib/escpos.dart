...
List<int> buildEscposReceipt(
  ...
  final showSubtotal = settings['receiptShowSubtotal'] != false;
  ...
  lines.add(Cmd.alignLeft);
  lines.add(Cmd.boldOn);
  lines.addAll([enc('Total Due: ${fnum(totalAmount)} Rs'), Cmd.lf]);
  lines.add(Cmd.boldOff);

  if (showSubtotal) {
    lines.addAll([
      enc(_formatLine('Subtotal:', '${fnum(subtotal)} Rs', maxChars)),
      Cmd.lf,
    ]);
  }
